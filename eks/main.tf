module "eks" {
  ...
  keycloak_db_credentials     = var.keycloak_db_credentials
...
module "vpc" {
  ...
  tags                   = var.tags[var.env]
  private_subnet_tags    = { karpenter_can_use = "true" }
}

module "keycloak_aurora_db" {
  source             = "git@github.com:leroykayanda/terraform-cloud-modules.git//aws/aurora?ref=1.2.37"
  service            = "keycloak"
  tags               = var.tags[var.env]
  env                = var.env
